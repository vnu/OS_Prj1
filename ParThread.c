 ...
 * 			It creates 4 threads that compresses and writes a segment of the input file to the output file.
...
int main(int argc, char **argv) {
	...
	if (argc != 3)//checking for number of arguments
	{
		...
		while (l==0)
		{
			...
			count++;
			//If the thread count is > 5 or if it is the end of the file
			if(count==5 || l==1)
			{
				int k=0;
				while(count!=0)
	...
	stop = end.tv_sec + end.tv_usec/1000000.0;
	runtime = stop - start;
	printf("\nRunning Time for ParThread\t: %f Seconds\n\n", runtime);
